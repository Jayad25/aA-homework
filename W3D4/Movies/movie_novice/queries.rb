...
def find_angelina
  ...
  x = Actor.find_by_name('Angelina Jolie')
  x
end
...
def top_titles
  ...
  # hint: use 'select' and 'where'
    title1 = Movie.where('score >= ?',9)
    title1.select(:id,:title)
...
def uma_movies
  ...
  # hint: use 'select', 'joins', 'where', and 'order'
   Movie.select(:id,:title,:yr).joins(:actors).where('name like ?','Uma Thurman').order('yr ASC')
   # .joins(:castings)
